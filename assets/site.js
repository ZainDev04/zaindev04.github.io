...
(function () {
  ...
  function initScroll() {
    ...
    function update() {
      ticking = false;
      head.classList.toggle('stuck', (window.scrollY || window.pageYOffset) > 8);
    }
  ...
  /* 8,130 pages sitting in one bucket, resolving into five populations.
     Monochrome: four greys and one white group. */
  function initSplit() {
    ...
    var GROUPS = [
      { n: 250, label: '3,365', hi: false },
      { n: 218, label: '2,920', hi: false },
      { n: 58,  label: '735',   hi: true  },
      { n: 46,  label: '588',   hi: false },
      { n: 40,  label: '522',   hi: false }
    ];
    var C_MUTED = '#5f5f5f', C_DOT = '#7a7a7a', C_HI = '#ffffff';
    var FONT = '400 11px Oswald, "Arial Narrow", sans-serif';
    ...
    function draw() {
      ...
      if (t > 0.55) {
        ...
        ctx.textAlign = 'center';
        GROUPS.forEach(function (g, k) {
          ctx.fillStyle = g.hi ? C_HI : C_MUTED;
